...
public interface Place extends Identifiable<String>, Configurable<Place> {
    ...
    /**
     * Navigates to a place (animated).
     */
    default void go(boolean push) {
        go(push, true);
    }
     ...
     *
     * @param push     <code>true</code> iff previous place must be kept into the
     *                 stack (allows returning later to the current place using
     *                 {@link PlaceManager#back()}).
     * @param animated <code>true</code> iff the the new screen should be shown using
